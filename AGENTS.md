...
- One TCP connection carries one file transfer or one text message.
- Portability matters: keep POSIX and Windows behavior split behind `_WIN32`.
- Compression support uses vendored LZ4 from `external/lz4/lz4.c`.
- Compiler warnings are enabled, but there is no dedicated lint target.
## Repository Map
- `src/hfile.c`: startup, Windows UTF-8 argv handling, and top-level dispatch.
- `src/cli.c`, `src/cli.h`: CLI parsing, usage text, and option structs.
- `src/client.c`, `src/client.h`: client connection flow, send path, and compression.
- `src/server.c`, `src/server.h`: listener, receive loop, file save, and acknowledgements.
- `src/protocol.c`, `src/protocol.h`: wire header encoding and file/compression prefix helpers.
- `src/net.c`, `src/net.h`: sockets, endian helpers, send/recv loops, and socket diagnostics.
- `src/fs.c`, `src/fs.h`: file I/O, basename/path helpers, and temp-file finalization.
- `src/helper.c`, `src/helper.h`: monotonic timing and perf reporting.
- `test/test_support.py`: tests for shared harness helpers.
- `test/test_cli.py`: CLI validation and client-side smoke tests.
- `test/test_transfer.py`: end-to-end transfer and raw protocol tests.
- `test/support/hf.py`: subprocess, temp-dir, protocol-define, and server helpers.
...
- No additional agent rule files were found during inspection.
## Build Commands
...
- `build.sh` records the last platform in `.build_platform`; switching native vs Windows deletes `build/`.
...
- CI uses direct CMake configure/build and then runs `python -m unittest -v test.test_hf`.
...
- `compile_commands.json` is generated, but there are no repo-defined static-analysis commands.
- Do not invent formatter or lint commands unless the user explicitly asks.
## Test Commands
### Build prerequisite
...
- `python3 -m unittest -v test.test_transfer.TestTransferProtocol`
### Single test method
...
- `python3 -m unittest -v test.test_transfer.TestTransferCLI.test_empty_file`
- `python3 -m unittest -v test.test_transfer.TestTransferProtocol.test_raw_file_transfer_two_phase_success`
- `python3 -m unittest -v test.test_transfer.TestTransferProtocol.test_protocol_rejects_invalid_magic`
### Test notes
...
- Reuse helpers in `test/support/hf.py` instead of open-coding subprocess, log, or socket logic.
- For CLI-only changes, run at least one CLI test.
- For client, server, protocol, net, or fs changes, run at least one CLI test and one transfer test.
- Run the full suite before finishing non-trivial protocol or runtime changes.
## Change Strategy
...
- In many source files, local headers come first and platform-specific system headers stay inside `_WIN32` guards.
...
- Enum/result constants and macros use uppercase identifiers such as `PARSE_ERR` and `HF_PROTOCOL_MAGIC`.
- Cleanup labels are uppercase, for example `CLEANUP`, `CLEAN_UP`, and `CLOSE_SOCK`.
...
- Keep borrowed string pointers `const char *` unless ownership changes.
- Encode and decode wire integers with helpers such as `encode_u64_be(...)` and `decode_u64_be(...)`.
### Error handling
...
- Distinguish EOF from hard I/O or protocol errors when receiving data.
- Preserve existing cleanup labels and control flow when adding new failure paths.
### Resource management
...
- Use fs helpers for basename/path/temp-name behavior instead of open-coded path parsing.
## Python Test Style
...
- Use 4-space indentation, `pathlib.Path`, explicit timeouts, and typed helper signatures.
...
- For protocol tests, prefer `protocol_define(...)` and the existing raw-send helpers over ad hoc socket code.
## Protocol And Runtime Notes
- `HF_PROTOCOL_HEADER_SIZE` is `13` bytes; current magic and version are `0x0429` and `0x02`.
- File transfers send file-name length, file-name bytes, and `content_size`, then wait for a one-byte ready ack before the body.
- The server ack byte is `0` for success and nonzero for failure; file transfers use both a ready ack and a final ack.
- Text messages use `HF_MSG_TYPE_TEXT_MESSAGE` and are capped at `256 KiB`.
- The current server/client flow expects file-transfer `payload_size == prefix_size + body_size`.
- Reject unsafe file names containing `/`, `\\`, or `..`.
- Compression mode uses raw or LZ4 blocks; preserve current block-header semantics unless the task intentionally changes the protocol.
...
- Fast transfer check: `python3 -m unittest -v test.test_transfer.TestTransferCLI.test_empty_file`
