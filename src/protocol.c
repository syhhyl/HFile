...
static void encode_u64_be(uint64_t v, uint8_t out[8]) {
...
static uint64_t decode_u64_be(const uint8_t in[8]) {
...
static int proto_res_frame_valid(const res_frame_t *frame) {
  if (frame == NULL)
    return 0;
  if (frame->phase != PROTO_PHASE_READY && frame->phase != PROTO_PHASE_FINAL)
    return 0;
  if (frame->status != PROTO_STATUS_OK &&
      frame->status != PROTO_STATUS_REJECTED &&
      frame->status != PROTO_STATUS_FAILED)
    return 0;
  if (frame->error_code > PROTOCOL_ERR_EOF)
    return 0;
  if (frame->status == PROTO_STATUS_OK) {
...
static protocol_result_t encode_res_frame(const res_frame_t *frame, uint8_t *out) {
...
static protocol_result_t decode_res_frame(res_frame_t *frame, const uint8_t *in) {
  uint16_t net_error_code = 0;
...
protocol_result_t send_res_frame(socket_t sock, const res_frame_t *frame) {
  ...
  ssize_t n = send_all(sock, buf, sizeof(buf));
  if (n < (ssize_t)sizeof(buf))
    return n >= 0 ? PROTOCOL_ERR_SHORT_WRITE : PROTOCOL_ERR_IO;
...
protocol_result_t recv_res_frame(socket_t sock, res_frame_t *frame) {
  ...
  ssize_t n = recv_all(sock, buf, sizeof(buf));
  if (n < (ssize_t)sizeof(buf))
...
protocol_result_t proto_send_payload(socket_t sock, const uint8_t *in, size_t len) {
  ...
  ssize_t n = send_all(sock, in, len);
  if (n < (ssize_t)len)
    return n >= 0 ? PROTOCOL_ERR_SHORT_WRITE : PROTOCOL_ERR_IO;
...
static protocol_result_t proto_recv_file_name_only(socket_t sock, char **file_name_out) {
  ...
  *file_name_out = NULL;
  ...
  char *file_name = (char *)malloc((size_t)file_name_len + 1);
  if (file_name == NULL)
...
protocol_result_t proto_recv_file_transfer_prefix(socket_t sock,
  ...
  *content_size_out = 0;
  ...
  char *file_name = NULL;
  protocol_result_t res = proto_recv_file_name_only(sock, &file_name);
  if (res != PROTOCOL_OK)
    return res;

  uint8_t szbuf[8];
  ssize_t n = recv_all(sock, szbuf, sizeof(szbuf));
